    ...
    suspend fun finish(r: BrokerResult) {
        ...
        if (r.wardScoped) {
            throw BrokerException(
                r.enrollBlockReason
                    ?: "Daily sync is set up per stake, by a stake-level leader. Your access covers " +
                    "your own unit, which you can already view here — there’s nothing to set up.",
            )
        }
        ...
        if (!r.stored && r.wouldDowngrade) {
            throw BrokerException(
                "This stake’s daily sync is already provided by ${r.existingProvider ?: "another leader"} " +
                    "with broader access. Your access is narrower, so it was left in place to avoid " +
                    "reducing coverage. No change is needed.",
            )
        }
