...
| 9 | Syncing banner (elapsed) + stale-credential banner | **Done** | `SyncingBanner` (1s timer), `StaleCredentialBanner` (revoked → re-enroll). |
| 10 | 5 tabs each accent color; single-stake query | **Done** | `DashboardTab` accents; `MembersRepository` `.eq(stake_id)` + exact `_columns`. |
| 11 | Skeleton loading + per-enrollment empty states | **Done** | `MemberListSkeleton`/`CardSkeleton`; `EnrollmentEmptyState` (no-role / revoked / active / generic). |
...
| 26 | Admin · Ops console | **Done** | `AdminScreen`/`AdminViewModel`: health/freshness/maintenance/links (`/admin/summary`), diagnostics (`/admin/diagnostics`), enrolled stakes (revoke + per-stake sync), GitHub Actions runs (+re-run) & changelog (`/admin/actions`), admins (invite `/admin/invite` + revoke rpc). Each panel loads independently. |
