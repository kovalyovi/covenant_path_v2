...
| A28 | **OTP identifier = Church USERNAME, not email (the enumeration-prevention phantom)** — the Church Okta org matches only the username: an email/unknown identifier gets Okta's phantom flow (menu offered, select accepted, "code sent", **no email ever sent**, every code "invalid" — wire-indistinguishable by design, probe-proven 2026-06-12; stranded Ken Packer's stake onboarding + the operator). Fix: (a) broker resolves a typed email → cached Church username via `church_identities` before identifying (returning users can keep typing their email); (b) unresolved email identifiers get `identifier_hint` in code_sent + username guidance appended to a rejected code; (c) the OTP lane rewrites the MFA-menu "choose a verification method" dead-end (challenge-less pending after a failed verify) to "Send a new code" guidance; (d) all 3 surfaces relabel the field "Church username" (username autofill semantics) with pre-send + post-send warnings on email-shaped input; (e) successful sends are audited (`otp_code_sent`) | ✅ offline `test_broker::test_otp_email_identifier_guidance` + `::test_otp_start_resolves_cached_email` (fail pre-fix) + Playwright `reauth-dialog` (A28 test) + vitest `mfaCopy.test.ts` · 🔶 native mirrors (`MfaCopy.swift`/`MfaCopy.kt`, Reauth sheets) via CI + AVD |
